...
class HostAuthority {
  ...
  hasAuthority(action) { return this.authoritative; }

  validateAction(action) { return this.authoritative; }

  applyInput(playerId, input) { return; }

  serializeState() {
    return { type: 'local', timestamp: performance.now() };
  }

  /* =================================================================
      発射リクエスト処理（Projectile / Energy / Summon）
     ================================================================= */
  ...
  handleFireRequest(data, peerId, inputId) {
    ...
    const spreadMult = this.game.passiveManager
      ? this.game.passiveManager.getSpreadMultiplier(peerId) : 1;
    ...
    for (let i = 0; i < pellets; i++) {
      ...
      const proj = new Projectile(
        this.game.scene, spawnPos, dir,
        peerId, pid, data.color || 0xffffff, data.weapon, mapHalf
      );
      ...
      /* ネットワークブロードキャスト */
      this._broadcastProjectile(peerId, pid, data.weapon,
        data.position, dir, data.color || 0xffffff, inputId);
    }
  ...
  handleBeamFireRequest(data, peerId, inputId) {
    ...
    const beamData = {
      ...
      damage: wp.damage,
      spread: 0, cooldown: 0, reflectCount: 0,
      plasmaBoomRadius: 0,
      criticalChance: 0, criticalDamageMultiplier: 2,
    };
    if (passive) passive.applyToBeam(beamData, peerId);

    /* 2Dレイキャスト */
    const result = this._beamRaycast(
      origin, dir, beamData.range, map, peerId, wp, beamData.width
    );
    ...
    if (result.hitPlayer) {
      ...
      if (victim && victim.alive) {
        const killer = this.game.players.get(peerId);
        ...
        const hitMsg = {
          type: 'hit', shooterId: peerId, targetId: result.hitPlayer,
          damage: beamDmg, shooterName: killer ? killer.name : '?',
        ...
        if (killed) this._trackKill(peerId, result.hitPlayer, data.weapon);
  ...
  _broadcastProjectile(ownerId, pid, weapon, pos, dir, color, inputId) {
    this.game.network.broadcast({
      type: 'proj_spawn', ownerId, pid, weapon,
  ...
  _validateFireRate(peerId, weapon) {
    ...
    const interval = wp.fireRate * 1000;
  ...
  _consumeAmmo(peerId, weapon) {
    ...
    if (wp && (wp.weaponType === 'energy' || wp.weaponType === 'beam')) {
      const heatState = this._getHeatState(peerId, weapon);
      const heatPerShot = Math.max(1, Math.ceil(
        (wp.heatCapacity || 40) * (wp.fireRate || 0.25) / 1.5
      ));
      heatState.heat = Math.min(heatState.maxHeat, heatState.heat + heatPerShot);
      if (heatState.heat >= heatState.maxHeat) heatState.overheated = true;
    ...
    const state = this.getAmmoState(peerId, weapon);
    if (!state) return;
    const cost = this.game.passiveManager
      ? this.game.passiveManager.getAmmoCost(peerId, 1) : 1;
  ...
  getAmmoState(peerId, weapon) {
    ...
    if (!this.ammoStates.has(key)) {
      ...
      const ammo = this.game.passiveManager
        ? this.game.passiveManager.getMagazineSize(peerId, baseAmmo) : baseAmmo;
  ...
  refillAmmo(peerId, weapon) {
    ...
    if (wp) {
      const ammo = this.game.passiveManager
        ? this.game.passiveManager.getMagazineSize(peerId, wp.maxAmmo) : wp.maxAmmo;
      this.ammoStates.set(key, { ammo, weapon });
      if (wp.heatCapacity) {
        this.heatStates.set(key, {
          heat: 0, maxHeat: wp.heatCapacity,
          coolingSpeed: wp.coolingSpeed || 0, overheated: false,
        });
      }
    }
  ...
  processHit(proj, victimId, weapon) {
    ...
    const dmgReduction = passive
      ? passive.getDamageReduction(victimId, 'projectile') : 1;
    damage *= dmgReduction;
    ...
    if (proj.lifeSteal) {
      ...
      if (shooter && shooter.alive) {
        shooter.health = Math.min(
          shooter.health + damage * proj.lifeSteal, shooter.maxHealth
        );
      }
    }
    ...
    const hitMsg = {
      ...
      damage, shooterName: killer ? killer.name : '?',
  ...
  processExplosion(proj, hitPlayers) {
    ...
    this.game.players.forEach((victim, id) => {
      ...
      const vPos = new THREE.Vector3(
        victim.position.x, CONFIG.playerHeight / 2, victim.position.z
      );
      ...
      if (dist < hitDist) {
        ...
        const hitMsg = {
          ...
          damage: expDamage, shooterName: killer ? killer.name : '?',
          targetName: victim.name, lethal: killed, weapon: proj.weapon,
          pos: { x: pos.x, y: pos.y, z: pos.z }, explosive: true,
        };
  ...
  _spawnGravityZone(pos, ownerId, color) {
    const zone = {
      position: pos.clone(), ownerId, color: color || 0x2200aa,
      age: 0, duration: 4, radius: 8, pullStrength: 8,
      damageTick: 5, tickInterval: 1, lastTick: 0,
    };
  ...
  _fireDroneMissile(droneProj) {
    const target = this._findNearestEnemy(
      droneProj.mesh.position, droneProj.ownerId
    );
    if (!target) return;
    ...
    const missile = new Projectile(
      this.game.scene, origin, toTarget,
      droneProj.ownerId, pid, 0xff4444, 'missile_drone'
    );
    missile.speed = 30;
    ...
    this.game.network.broadcast({
      type: 'proj_spawn',
      ownerId: droneProj.ownerId, pid, weapon: 'missile_drone',
      pos: { x: origin.x, y: 0, z: origin.z },
      dir: { x: toTarget.x, y: 0, z: toTarget.z },
      color: 0xff4444, inputId: pid,
      homing: true, homingTarget: target.id,
    });
  }
  ...
  handleHostProjectiles(dt) {
    ...
    for (let i = projs.length - 1; i >= 0; i--) {
      ...
      if (proj.alive && proj.isHoming && proj.homingTargetId) {
        ...
        if (target && target.alive) {
          ...
          if (dist > 0.5) {
            toTarget.normalize();
            proj.velocity.lerp(
              toTarget.multiplyScalar(proj.speed), dt * proj.homingStrength
            );
            proj.velocity.y = 0;
          }
        } else {
      ...
      if (map) {
        ...
        const pr = CONFIG.projectileRadius || 0.3;

        if (Math.abs(proj.mesh.position.x) > half ||
            Math.abs(proj.mesh.position.z) > half) {
        ...
        for (let wi = 0; wi < walls.length; wi++) {
          ...
          if (Math.abs(proj.mesh.position.x - wx) < wHalfX &&
              Math.abs(proj.mesh.position.z - wz) < wHalfZ) {
      ...
      this.game.players.forEach((victim, id) => {
        ...
        const hitDist = (CONFIG.playerSize * 0.5 +
          ((proj.wp && proj.wp.hitRadius) || 0.8)) * (proj.passiveSizeMult || 1);
        ...
        if (proj.mesh.position.distanceTo(vPos) < hitDist) {
          ...
          if ((proj.wp && proj.wp.explosive) || proj.explosiveAmmo) {
            ...
            this.game.effectManager.spawnHitEffect(
              proj.mesh.position.clone(), proj.color || 0xffffff
            );
            this.game.network.broadcast({
              type: 'hit_effect',
              pos: { x: proj.mesh.position.x, y: proj.mesh.position.y, z: proj.mesh.position.z },
  ...
  _beamRaycast(origin, dir, maxRange, map, ownerId, wp, customHitRadius) {
    ...
    if (map && map.walls) {
      for (const wall of map.walls) {
        ...
        const dist = this._rayAABB2D(origin, dir,
          wx - hx, wz - hz, wx + hx, wz + hz
        );
